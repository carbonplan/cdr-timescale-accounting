...
def calculate_new_variables(df, concentration_driven):
    # New variables
    df['fast_c']=(df['LL_ocean_c']+ df['HL_ocean_c']+
                   df['IO_ocean_c']+
                   df['veg_c']+df['detritus_c']+df['soil_c']+
                   df['atmos_co2']
                 )
...
def condense_output(df, deployment_year, concentration_driven=False,
                    spinup_cutoff=-100, variables_to_drop=variables_to_drop):
